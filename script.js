document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelector(".root").innerHTML = `
      <root>
        ...
        <main>
          ...
          <div class="main_container_sections_byTopic">
            <section class="home_dashboard">
              ...
              <div class="main_cards_information">
                ...
                <div class="card">
                  ...
                  <div class="comparation_with_lastValue">+1°C</div>
                </div>
                ...
                <div class="card">
                  ...
                  <div class="comparation_with_lastValue">-9%</div>
                </div>
                ...
                <div class="card">
                  ...
                  <div class="comparation_with_lastValue">-10 CO2e</div>
                </div>
                ...
                <div class="card">
                  ...
                  <div class="comparation_with_lastValue">+1 /Kwh</div>
                </div>
              </div>
            </section>
            ...
            <section class="temperature_dashboard">
              ...
              <div class="temperature_container_all_cards"></div>
            </section>
    
            <section class="humidity_dashboard">
              <h1>Humedad</h1>
              <div class="all_humidity_cards_container"></div>
            </section>
    
            <section class="presencie_dashboard">
              <h1>Presencia</h1>
              <div class="all_presencie_cards_container"></div>
            </section>
    
            <section class="presencie_dashboard">
              <h1>CO2</h1>
              <div class="all_airs_cards_container"></div>
            </section>
          </div>
    
             <div class="popupWindownOsorio">
        ...
        <div class="container_chart">
          ...
          <iframe
            src="http://192.168.0.105/scada-vis/trends"
            frameborder="0"
            class="chart_popup"
          >
          </iframe>
        ...
        <aside class="right-aside">
          ...
          <div class="sensors_name_label">
            <h1>Tabla de Sensores</h1>
            <button>Ver</button>
          </div>
    
          <div class="charts_panel_button">
            <h1>Graficos</h1>
              <button class="viewCharts">Ver</button>
          </div>
    
          <div class="schedule_panel_button">
            <h1>Horarios</h1>
            <button>Ver</button>
          </div>
  ...
  const dayWeek = new Intl.DateTimeFormat("es-Es", { weekday: "long" }).format(
    today
  );
  ...
  document
    ...
    .forEach((m) => (m.textContent = `${month} ${today.getFullYear()}`));
  document.querySelector(
    ".dayWeek"
  ).textContent = `${dayWeek}, ${month} ${today.getDate()}`;
  ...
  const kesplus = Object.values(data).slice(8, 38);
  const mdt = Object.values(data).slice(38, 56);
  ...
  const prsnc_addresses = ["0/0/1", "0/0/2"];
  ...
  //   TARGETS TO INSERT HTML

  const tempTargetContainer = document.querySelector(
    ".temperature_container_all_cards"
  );
  const humdTargetContainer = document.querySelector(
    ".all_humidity_cards_container"
  );
  const presencieTargetContainer = document.querySelector(
    ".all_presencie_cards_container"
  );
  const airsTargetContainer = document.querySelector(
    ".all_airs_cards_container"
  );

  //   IFRAME WIDGET CHARTS

  const chartWidgetWidget = (document.querySelector(
    ".container_chart"
  ).style.visibility = "hidden");
  const dataSensorWindown = (document.querySelector(
    ".popupWindownOsorio"
  ).style.visibility = "hidden");
  const containerDataSensorTarget = document.querySelector(".container_popup");

  // CONDITIONALS CONTROL WIDGETS IFRAME

  const showChart = false;
  const showData = false;

  document.querySelector(".closeChart").addEventListener("click", () => {
    document.querySelector(".container_chart").style.visibility = "hidden";
  });

  document.querySelector(".viewCharts").addEventListener("click", () => {
    document.querySelector(".container_chart").style.visibility = "visible";
  });

  //   SENSORS BUTTON

  const sensorsButtons = document.querySelectorAll(
    ".card_info_quantity_sensor"
  );

  sensorsButtons.forEach((button) => {
    button.addEventListener("click", () => {
      const sensorText = button.children[0]?.children[1]?.textContent;
      const dataSensorWindown = (document.querySelector(
        ".popupWindownOsorio"
      ).style.visibility = "visible");

      if (sensorText === "Presencia C v2") {
        containerDataSensorTarget.innerHTML = presenciaC
          .map((data) => {
            return `
                <div class="card_sensor_data">
                  <h1>${data.value}</h1>
                  ${
                    temp_addresses.includes(data.address)
                      ? "<p>Temperatura</p>"
                      : ""
                  }
                  ${air_addresses.includes(data.address) ? "<p>CO2</p>" : ""}
                  ${
                    hmd_addresses.includes(data.address) ? "<p>Humedad</p>" : ""
                  }
                  ${
                    prsnc_addresses.includes(data.address)
                      ? "<p>Presencia</p>"
                      : ""
                  }
                  <p>Dirección de grupo: ${data.address}</p>
                  <button>Cambiar Nombre</button>
                </div>`;
          })
          .join("");
      } else if (sensorText === "Sensor MDT") {
        containerDataSensorTarget.innerHTML = mdt
          .map((data) => {
            return `
                <div class="card_sensor_data">
                  <h1>${data.value}</h1>
          ...
          .join("");
      } else if (sensorText === "KESPLUS") {
        containerDataSensorTarget.innerHTML = kesplus
          .map((data) => {
            return `
                <div class="card_sensor_data">
                  <h1>${data.value}</h1>
  ...
  //HIDE WINDOWN POPUP DATA SENSOR
  ...
  temp_addresses.map((t) => {
    grp.listen(t, function (object, state) {
      ...
      document.querySelector(".temp_average").textContent = `${(
        temperatures.reduce((average, current) => average + current, 0) /
        temperatures.length
      ).toFixed(2)}°C`;
      document.querySelector(".temp_max").textContent = `${Math.max(
        ...temperatures
      )}°C`;
      document.querySelector(".temp_min").textContent = `${Math.min(
        ...temperatures
      )}°C`;
      tempTargetContainer.innerHTML = temperatures
        .map(
          (temp, index) =>
            `
  ...
  hmd_addresses.map((t) => {
    grp.listen(t, function (object, state) {
      ...
      humdTargetContainer.innerHTML = humidities
        .map(
          (humd, index) =>
            `
  ...
  prsnc_addresses.map((t) => {
    grp.listen(t, function (object, state) {
      ...
      presencieTargetContainer.innerHTML = presencies
        .map(
          (pre, index) =>
            `
                       <div class="humidity_card">
                          ...
                          <path fill="currentColor"
                              ...
                              <div class="data_content_hmd">
                                  ${
                                    pre == true
                                      ? `<h1 class="hmd_value">Activado</h1>`
                                      : `<h1 class="hmd_value">Apagado</h1>`
                                  }
                                  <p class="hmd_address">Sensor ${index + 1}</p>
                                  <p>${prsnc_addresses[index]}</p>
                              </div>
                          </div>
                  `
        )
  ...
  air_addresses.map((t) => {
    grp.listen(t, function (object, state) {
      ...
      airsTargetContainer.innerHTML = airs
        .map(
          (co, index) =>
            `
                      <div class="humidity_card">
